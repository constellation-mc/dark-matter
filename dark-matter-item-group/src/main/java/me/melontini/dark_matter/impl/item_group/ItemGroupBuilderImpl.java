...
public class ItemGroupBuilderImpl implements ItemGroupBuilder {
  ...
  public ItemGroupBuilderImpl(Identifier identifier) {
    if (!FabricLoader.getInstance().isModLoaded("fabric-item-group-api-v1"))
      DarkMatterLog.warn("Building {} ItemGroup without Fabric Item Groups", identifier);
  ...
  public ItemGroup build() {
    ...
    builder.displayName(Objects.requireNonNullElseGet(
        this.displayName,
        () -> Text.translatable("itemGroup." + this.identifier.toString().replace(':', '.'))));
