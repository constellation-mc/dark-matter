...
public class ClientRecipeBookUtils {
  ...
  private static final Map<RecipeBookCategory, List<RecipeBookGroup>> GROUPS_FOR_CATEGORY =
      new IdentityHashMap<>();

  private static final Map<RecipeBookCategory, Supplier<List<RecipeBookGroup>>> VANILLA_CATEGORIES =
      Utilities.supply(new IdentityHashMap<>(), map -> {
  ...
  public static <T extends Recipe<?>> Event<RecipeGroupLookupEvent<T>> forType(
      RecipeType<T> type, boolean create) {
    if (create) {
      return Utilities.cast(EVENTS.computeIfAbsent(
          type,
          type1 -> EventFactory.createArrayBacked(
              RecipeGroupLookupEvent.class,
              recipeGroupLookupEvents -> (id, recipe, registryManager) -> {
                RecipeBookGroup group;
                for (RecipeGroupLookupEvent<T> event : recipeGroupLookupEvents) {
                  if ((group = event.onRecipeBookGroupLookup(id, (T) recipe, registryManager))
                      != null) {
                    return group;
                  }
                }
  ...
  public static void registerGroups(
      @NonNull RecipeBookCategory category, @NonNull List<RecipeBookGroup> groups) {
    getGroupsForCategory(category).addAll(groups);
  }

  public static void registerGroups(
      @NonNull RecipeBookCategory category, int index, @NonNull List<RecipeBookGroup> groups) {
  ...
  public static void addToSearchGroup(
      @NonNull RecipeBookGroup searchGroup, @NonNull List<RecipeBookGroup> groups) {
    List<RecipeBookGroup> groupList =
        RecipeBookGroup.SEARCH_MAP.computeIfAbsent(searchGroup, group -> new ArrayList<>());
    groupList.addAll(groups);
  }

  public static void addToSearchGroup(
      @NonNull RecipeBookGroup searchGroup, int index, @NonNull List<RecipeBookGroup> groups) {
    List<RecipeBookGroup> groupList =
        RecipeBookGroup.SEARCH_MAP.computeIfAbsent(searchGroup, group -> new ArrayList<>());
  ...
  public static Optional<List<RecipeBookGroup>> getGroups(RecipeBookCategory category) {
    return Optional.ofNullable(GROUPS_FOR_CATEGORY.get(category))
