...
public class NbtUtil {
  ...
  public static @NotNull NbtCompound writeInventoryToNbt(
      NbtCompound nbt, @NotNull Inventory inventory) {
   ...
   */
  public static @NotNull NbtCompound writeInventoryToNbt(
      String key, NbtCompound nbt, @NotNull Inventory inventory) {
    ...
    for (int i = 0; i < inventory.size(); ++i) {
      ...
      if (!itemStack.isEmpty()) {
        nbtList.add(
            itemStack.writeNbt(NbtBuilder.create().putByte("Slot", (byte) i).build()));
      }
    }
    nbt.put(key, nbtList);
    return nbt;
