...
public class FakeWorld {
  ...
  public static final Supplier<ClientWorld> INSTANCE = Suppliers.memoize(() -> {
    ...
    try {
      ...
      ClientPlayNetworkHandler networkHandler = new ClientPlayNetworkHandler(
          ...
          null);
      networkHandler.combinedDynamicRegistries =
      ...
      return new ClientWorld(
          ...
          World.OVERWORLD,
          regs.getPrecedingRegistryManagers(ServerDynamicRegistryType.DIMENSIONS)
  ...
  private static CombinedDynamicRegistries<ServerDynamicRegistryType> getRegistries() {
    ...
    CombinedDynamicRegistries<ServerDynamicRegistryType> cdr2 =
        bootstrapBuiltin(combinedDynamicRegistries);

    DynamicRegistryManager.Immutable immutable =
        cdr2.getPrecedingRegistryManagers(ServerDynamicRegistryType.DIMENSIONS);
    ...
    DimensionOptionsRegistryHolder.DimensionsConfig dimensionsConfig =
        preset.toConfig(new SimpleRegistry<>(RegistryKeys.DIMENSION, Lifecycle.stable()));

    return cdr2.with(
        ServerDynamicRegistryType.DIMENSIONS, dimensionsConfig.toDynamicRegistryManager());
  }

  private static CombinedDynamicRegistries<ServerDynamicRegistryType> bootstrapBuiltin(
      CombinedDynamicRegistries<ServerDynamicRegistryType> cdr) {
    RegistryWrapper.WrapperLookup pain = BuiltinRegistries.createWrapperLookup();

    List<? extends RegistryKey<? extends Registry<?>>> keys =
    ...
    for (RegistryKey<? extends Registry<?>> key : keys) {
      SimpleRegistry<Object> registry =
          new SimpleRegistry<>(Utilities.cast(key), Lifecycle.stable());

      pain.getOptionalWrapper(key).ifPresent(impl -> impl.streamEntries()
          .forEach(ref -> registry.add(ref.registryKey(), ref.value(), Lifecycle.stable())));

      registry.freeze();
      regs.add(registry);
    }
    DynamicRegistryManager.Immutable immutable1 =
        new DynamicRegistryManager.ImmutableImpl(ImmutableList.copyOf(regs)).toImmutable();
