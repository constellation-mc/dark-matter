...
public class AsmTransformerPlugin implements IPluginPlugin {
  ...
  private final Map<Tuple<String, String>, Set<IAsmTransformer>> transformers =
  ...
  public void beforeApply(
    ...
    ClassNode mixinNode = mixinInfo.getClassNode(
    ...
    for (Type type : types) {
      ...
      Set<IAsmTransformer> transformers = this.transformers.computeIfAbsent(
          Tuple.of(mixinInfo.getClassName(), targetClassName), k -> new LinkedHashSet<>());
      transformers.add(
          (IAsmTransformer) Reflect.setAccessible(cls.getDeclaredConstructor()).newInstance());
      DarkMatterLog.debug(
          "Added transformer {} to mixin {}", type.getClassName(), mixinInfo.getClassName());
    }

    Set<IAsmTransformer> transformers =
        this.transformers.get(Tuple.of(mixinInfo.getClassName(), targetClassName));
