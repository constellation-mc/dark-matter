...
public class UnsafeUtilsTest {
  ...
  void testDefineClass() {
    ...
    node.visit(
        Opcodes.V17, Opcodes.ACC_PUBLIC, cls, null, Type.getInternalName(Object.class), null);

    InstructionAdapter adapter =
        new InstructionAdapter(node.visitMethod(Opcodes.ACC_PUBLIC, "<init>", "()V", null, null));
    ...
    node.accept(writer);
    Class<?> defined = UnsafeUtils.defineClass(
