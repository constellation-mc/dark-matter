...
public class UnsafeInternals {
  ...
  private static final Supplier<MethodHandles.Lookup> TRUSTED_LOOKUP =
      Suppliers.memoize(() -> supply(() -> UnsafeUtils.getReference(
          MethodHandles.Lookup.class.getDeclaredField("IMPL_LOOKUP"), null)));
  private static final Supplier<MethodHandle> DEFINE_CLASS =
      Suppliers.memoize(() -> supply(() -> TRUSTED_LOOKUP
          .get()
          .findVirtual(
              ClassLoader.class,
              "defineClass",
  ...
  public static void setReference(@NonNull Field field, Object o, Object value) {
    ...
    boolean isVolatile =
        Modifier.isVolatile(field.getModifiers()) || Modifier.isFinal(field.getModifiers());
    if (isVolatile) {
      getUnsafe()
          .putObjectVolatile(
              isStatic ? getUnsafe().staticFieldBase(field) : o,
              isStatic
                  ? getUnsafe().staticFieldOffset(field)
                  : getUnsafe().objectFieldOffset(field),
              value);
    } else {
      getUnsafe()
          .putObject(
  ...
  public static <T> T getReference(@NonNull Field field, @Nullable Object o) {
    ...
    boolean isVolatile =
        Modifier.isVolatile(field.getModifiers()) || Modifier.isFinal(field.getModifiers());
    if (isVolatile) {
      return Utilities.cast(getUnsafe()
          .getObjectVolatile(
              isStatic ? getUnsafe().staticFieldBase(field) : o,
              isStatic
                  ? getUnsafe().staticFieldOffset(field)
