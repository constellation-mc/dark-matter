...
public class MapperInternals {
  ...
  public static String mapField(
      @NonNull String owner, @NonNull String field, @NonNull String desc) {
    return RESOLVER.mapFieldName("intermediary", owner, field, desc);
  }

  public static String mapMethod(
      @NonNull String owner, @NonNull String method, @NonNull String desc) {
  ...
  public static String mapDescriptor(@NonNull String arg) {
    if (arg.startsWith("L"))
      arg = "L"
          + RESOLVER
              .mapClassName("intermediary", arg.substring(1, arg.length() - 1).replace('/', '.'))
              .replace('.', '/')
          + ';';
    if (arg.startsWith("["))
      arg = "[L"
          + RESOLVER
              .mapClassName("intermediary", arg.substring(2, arg.length() - 1).replace('/', '.'))
              .replace('.', '/')
          + ';';
    return arg;
  }
  ...
  public static String unmapDescriptor(@NonNull String arg) {
    if (arg.startsWith("L"))
      arg = "L"
          + RESOLVER
              .unmapClassName("intermediary", arg.substring(1, arg.length() - 1).replace('/', '.'))
              .replace('.', '/')
          + ';';
    if (arg.startsWith("["))
      arg = "[L"
          + RESOLVER
              .unmapClassName("intermediary", arg.substring(2, arg.length() - 1).replace('/', '.'))
              .replace('.', '/')
          + ';';
    return arg;
  }
